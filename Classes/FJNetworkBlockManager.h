...
- (void)sendRequest:(NSURLRequest*)req                          //what do you want?
    ...
    completionBlock:(void(^)(NSData* result))completionBlock    //called on success, cannot be nil
       failureBlock:(void(^)(NSError* error))errorBlock;        //called on errors, cannot be nil (3 attempts made before error block is called)
